...
document.addEventListener('DOMContentLoaded', function(){
	template = document.querySelector<HTMLTemplateElement>("#membergate-modal-template");
})
document.addEventListener("click",
	function(e:MouseEvent){
		if(!template) return;
		//@ts-ignore
		if(e.target.matches("a[href*='membergate_protect'], a[href*='membergate_protect'] *")){
			e.preventDefault()	
			const el = e.target as HTMLElement
			const anchorEl = el.nodeName == "A" ? el as HTMLAnchorElement : el.closest<HTMLAnchorElement>('a[href*="membergate_protect"')
...
function showModal(settings){
	...
	const replaceTextWithSettings = (el:HTMLElement)=>replaceText(settings,el)
	...
	modal.querySelectorAll("[data-replace-text]").forEach(replaceTextWithSettings)
	modal.querySelectorAll("[data-replace-value]").forEach(replaceValueWithSettings)
...
function handleModalClickEvents(e:MouseEvent){
	...
	const el = target.matches('[data-action]') ? target : target.closest("[data-action]") as HTMLElement
	if(!el) return;
	switch(el.dataset.action){
		case "close": return removeModal();
